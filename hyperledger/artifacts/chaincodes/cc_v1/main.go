...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	case "approveTokenRequest":
		return s.approveTokenRequest(APIstub, args)
	case "debug":
		return s.debug(APIstub)
	case "initLedger":
...
func (s *SmartContract) debug(APIstub shim.ChaincodeStubInterface) sc.Response {
	logger.Infof("loop done")
	return shim.Success(nil)
}
//args == 0. Id , 1.username , 2.Amount , 3.remark
func (s *SmartContract) approveTokenRequest(APIstub shim.ChaincodeStubInterface , args []string) sc.Response {

	if len(args) != 4 {
		return shim.Error("Incorrect number of arguments. Expecting 5")
	}

	logger.Infof("approveTokenRequest function in started invoking")
	...
	tnxBytes, _ := json.Marshal(tnx)
	APIstub.PutState(args[0], tnxBytes)
	ledgerKey++

	logger.Infof("amount converted to int" , amount)
	logger.Infof("loop started")
	power := 1 
	for amount > 0 {
		
		tokens := amount % 10 
		tokStruct := tokenDetails {
			Token : power,
			Count : tokens,
		}
		strPower := strconv.Itoa(power)
		tokBytes, err := APIstub.GetPrivateData("tokens", strPower)
		if err != nil {
			return shim.Error("Failed to get token data: " + err.Error())
		}
		if tokBytes == nil {
			logger.Infof("previous data is nil")
		}

		if tokBytes != nil {
			var prevTok tokenDetails
			err := json.Unmarshal(tokBytes, &prevTok)
			if err != nil {
				return shim.Error("Failed to unmarshal prev token data: " + err.Error())
			}
			logger.Infof("previous token data: " , prevTok.Count , prevTok.Count)
			tokStruct.Count += prevTok.Count
		}
		tokBytes , err = json.Marshal(tokStruct)
		if err != nil {
			return shim.Error("Failed to marshal updated token data: " + err.Error())
		}

		logger.Infof("updating token" , tokStruct.Token , tokStruct.Count)
		err = APIstub.PutPrivateData("tokens", strPower, tokBytes)
		if err != nil {
			logger.Infof("failed to insert token data" , err)
			return shim.Error("error in putting in private data " + err.Error())
		}
		logger.Infof("updated token" , power , tokStruct.Count)
